...
def get_sigcd_from_prv_sgg_name(prv_name, sgg_name):
    ...
    m = re.compile(u"^창원시")
    if m.search(sgg_name): # 창원, 마산, 진해 통합시때문에 구 이름 앞에 이상한게 붙어 있다.
    ...
    if lvl2_cd is None:
        print "ERROR: get_sigcd_from_prv_sgg_name : %s" % sgg_name
...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        if len(info_from_filename) == 2 and not (election_type == u'구시군장' or election_type == u'기초의원비례대표'):
            ...
            precinct_name = info_from_filename[2].replace(u"경상남도_","").replace(sgg_name + "_","") # 경상남도_거제시_거제시제2 이런 식으로 되어 있음.
            election_level = 2
            column_offset = -1
        else:
