...
def get_sigcd_from_prv_sgg_emd_name(prv_name, sgg_name, emd_name):
    ...
    # '야무지고 탐스럽게 살기 좋은 마을'을 만들어 가자는 뜻으로, 동명을 '도담'으로 정했다. 2012년 6월 30일까지 충청남도 연기군 남면 방축리, 종촌리, 고정리, 갈운리, 진의리 일부였다.
    if emd_name == u'도담동':
        emd_name = u'연기면'
...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        for sheet in wb.sheets():
            ...
            if sheet.name == u'세종시장선거':
                election_type = u'시장선거'
                sgg_name      = u'세종시' 
                precinct_name = u'세종특별자치시'
                column_offset = -1
                election_level = 1
            elif sheet.name == u'세종시교육감선거':
                election_type = u'교육감'
                sgg_name      = u'세종시' 
                precinct_name = u'세종특별자치시'
                column_offset = -1
                election_level = 1
            elif sheet.name == u'세종시의원선거':
                election_type = u'시의원'
                sgg_name      = u'세종시' 
                precinct_name = u'' # extract from header
                column_offset = 0
                election_level = 2
            elif sheet.name == u'비례대표세종시의원선거':
                election_type = u'광역비례대표'
                sgg_name      = u'세종시' 
                precinct_name = u'세종특별자치시'
                column_offset = -1
                election_level = 2
            ...
            print "==============================="
            print "%s %s %s" % (election_type, sgg_name, precinct_name)
            print "==============================="

            sheet_count = sheet_count + 1

            # 이 경우에 주의할 것은 시의원 선거임.
            ...
            for rownum in xrange(sh.nrows):
                ...
                if election_type == u'시의원' and values[0].decode('utf8') != "" and precinct_name != values[0].decode('utf8'):
                    # 시의원의 경우 header가 반복된다.
                    precinct_name = values[0].decode('utf8')
                    header_flag = True
                ...
                if header_flag: 
                    ...
                    if (values[3+column_offset] == "" or values[3+column_offset].decode("utf8") == precinct_name) and values[6+column_offset][:4] != "":
                        if split_flag:
                            split_flag = False
                            header_flag = False
                            continue
                        ...
                        for cn in values[5+column_offset:-3+bigo_offset]:
                            ...
                            if len(cn.split("\n")) > 1 and not split_flag:
                                ...
                                if election_type == u"광역비례대표" or election_type == u'기초비례':
                                    party = cn.replace("\n", "")
                                    name  = party
                                else:
