...
def get_sigcd_from_prv_sgg_name(prv_name, sgg_name):
    ...
    if sgg_name == u'덕진구' or sgg_name == u'완산구': #전주시덕진구와 완산구 예외처리.
...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        election_type = info_from_filename[0]  # 구시군의원 이런 식으로 되어 있음. 
        ...
        if len(info_from_filename) == 2 and not (election_type == u'기초비례' or election_type == u'시장군수'):
        ...
        elif len(info_from_filename) == 2: # 시장군수 및 기초비례의 경우.
            ...
            election_level = 2
            column_offset = -1
            if election_type == u"시도의원" and sgg_name == u"김제시":
                column_offset = 0 #하필이면 김제시는 한 칸 밀려있다.
        elif len(info_from_filename) == 4:
            # 전주시덕진구 등의 경우에 이름이 이상하다.
            sgg_name = info_from_filename[3] # 전주시덕진구 또는 완산구
