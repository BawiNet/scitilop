...
def get_sigcd_from_prv_sgg_name(prv_name, sgg_name):
    ...
    sgg_name = sgg_name.replace(u'포항시','')
...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        base_filename = file_in_dir[:file_in_dir.rfind(".")]
        sgg_name = u""

        info_from_filename = base_filename.split("-")
        election_type = info_from_filename[1][3:] # 읍면동별 개표자료-01_시도지사-경상북도

        if len(info_from_filename) == 3 and not (election_type == u'기초의원비례대표' or election_type == u'구시군장'):
            # 광역레벨의 경우
            sgg_name      = "" # will be imputed in the actual excel sheet
            precinct_name = info_from_filename[2]
            election_level = 1
            column_offset = 0  # first column is sgg_name
        elif len(info_from_filename) == 3: 
            # 기초의원비례대표 또는 구시군장(part)
            sgg_name = info_from_filename[2][3:] # 읍면동별 개표자료-02_구시군장-03 울릉군.xls
            precinct_name = sgg_name # 같은 선거구
            election_level = 2
            column_offset = -1
            if (sgg_name in [u'구미시', u'영천시', u'의성군']) and election_type == u'구시군장':
                column_offset = 0 # this has sgg_name
            if (sgg_name in [u'의성군']):
                column_offset = 0
        elif len(info_from_filename) == 4:
            if election_type in [u'구시군장', u'기초의원비례대표']:
                sgg_name = info_from_filename[3][3:] # 01 포항시북구
                precinct_name = sgg_name
            else:
                sgg_name = info_from_filename[2][3:] # 
                precinct_name = info_from_filename[3]
            election_level = 2
            column_offset = -1
            if sgg_name in [u'포항시남구', u'경주시', u'김천시', u'구미시', u'고령군', u'의성군'] and election_type == u'시도의원':
                column_offset = 0 # this has precinct_name first column
            if sgg_name in [u'구미시',u'경산시',u'고령군'] and election_type == u'구시군의원' and not (precinct_name in [u'구미시가선거구', u'경산시마선거구', u'경산시라선거구', u'경산시다선거구']):
                column_offset = 0
        else:
        ...
        for sheet in wb.sheets():
            ...
            if (sheet.name != "Sheet1") and election_level == 1:
                ...
                sgg_name = sheet.name[2:] # 23강남구 이렇게 되어 있음.
            ...
            for rownum in xrange(sh.nrows):
                ...
                if header_flag: 
                    header_index = header_index + 1 # you count from the header. 3 header lines usually present.
                    ...
                    if column_offset >= 0 and (election_level == 1 or election_type == u"기초의원비례대표"): 
                    ...
                    if column1 == u"합계" or column1 == u"관외사전투표" or column1 == u"거소우편투표" or column1 == u"잘못투입·구분된투표지":
                        ...
                        # 율곡동: 2014년 1월 2일 남면 용전리, 옥산리·봉천리·초곡리·운남리 일부, 농소면 신촌리·월곡리 
                        if (sgg_name == u"김천시" and emd_name == u"율곡동"):
                            emds = [u'남면', u'농소면']
                        else:
                            emds = [emd_name]
#                        emds = [emd_name]
