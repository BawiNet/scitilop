...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        # 부산광역시는 모두 xlsx파일이고, 전부 쉬트 안에 들어가 있다.

        info_from_filename = base_filename.split("-")
        election_type = info_from_filename[1] # 부산-광역비례-개표진행상황(읍면동별) 이렇게 되어있음.

        if election_type == u'시장' or election_type == u'교육감' or election_type == u'광역비례':
            # 광역레벨의 경우
            sgg_name      = "" # 이 경우엔 심지어 한 쉬트에 있어서 순차적으로 헤더를 잘 포착해야한다-_-
            precinct_name = u"부산광역시"
            election_level = 1
            column_offset = -1 # 여전히 sheet별로 시군구로 정리되어 있다.
        else:
            sgg_name = "" # 이 경우에는 쉬트의 이름으로 구분해야한다.
            precinct_name = "" # 이 경우에도 쉬트의 이름으로 구분해야한다.
            election_level = 2
            column_offset = -1

        # DEBUG-POINT1
        if DEBUG_POINT1: 
            print "%s -- %s -- %s -- %s" % (base_filename.encode("utf8"), sgg_name.encode("utf8"), precinct_name.encode("utf8"), election_type.encode("utf8"))
            raw_input()
            continue;
        ...
        for sheet in wb.sheets():
            ...
            if election_level == 1:
                "광역레벨은 쉬트 이름이 의미가 없다."
            elif election_level == 2:
                precinct_name = sheet.name
                # sgg_name은 두 번을 헤더에서 스플릿해서 갖아야한다.
            ...
            for rownum in xrange(sh.nrows):
                # 부산시의 경우 광역에서 쉬트별로가 아니라 그냥 테이블이 붙어있다.
                # 후보는 한 번 추가해야하므로, candidate_added = False인 경우에만 하자.
                # 이 경우 loop을 다 돈 다음에 candidate_added = True다.
                ...
                test_header = values[0].decode("utf8")

                if (test_header.find("[") >= 0):
                    # reset
                    header_index = 0 # first header line
                    header_flag = True 

                if header_flag: 
                    header_index = header_index + 1 # you count from the header. 3 header lines present.

                    if header_index == 1:
                        # 두 번째 헤더에서 뽑아내야한다.
                        # 항상 세 번째 대괄호다.
                        header = values[0].decode("utf8")
                        s1 = header.replace("[","*",2).find("[")
                        s2 = header.replace("]","*",2).find("]")
                        sgg_name = header[s1+1:s2]
                    ...
                    if (values[3+column_offset] == "" ) and values[6+column_offset][:4] != "":
                        ...
                        for cn in values[5+column_offset:-3+bigo_offset]:
                            ...
                            if election_level == 1 or election_type == u"기초비례": #광역레벨일 경우에 그렇게 써줘야한다.
                                ...
                                if not candidate_added: # this should happen only once.
                                    candidate_id[index] = save_candidate_info(
                                        election_name, 
                                        election_type, 
                                        election_level, 
