...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        # 서울특별시의 경우 파일형식이 아무 상관이 없고, 첫번째 줄부터 type이다.

        info_from_filename = base_filename.split("-")
        election_type = info_from_filename[0] 

        if election_type == u'광역의원비례대표(수정요)':
            election_type = u'광역의원비례대표' 
        ...
        if len(info_from_filename) == 2 and not (election_type == u'기초의원비례대표' or election_type == u'구시군장'):
            ...
            # 기초의원비례대표 또는 구시군장.
            sgg_name = info_from_filename[1] # 구시군장-남구
            precinct_name = sgg_name # 같은 선거구
            election_level = 2
            column_offset = -1
        elif len(info_from_filename) == 3:
            sgg_name      = info_from_filename[1] # 시도의원-남구-남구1
