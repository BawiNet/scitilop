...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        if len(info_from_filename) == 2:
            # 교육감, 도지사, 광역의원비례대표
            election_type = info_from_filename[1] 
        ...
        elif len(info_from_filename) == 1:
            # 도의원
            election_type = u"시도의회의원"
            sgg_name = "" # need to extract from header 
            precinct_name = info_from_filename[0]
            election_level = 2
            column_offset = -1
        elif len(info_from_filename) == 3:
            # 교육의원
            election_type = u'교육의원'
            precinct_name = info_from_filename[2]
            sgg_name = info_from_filename[2].split("_")[1]
            election_level = 2
            column_offset = -1
        else:
        ...
        for sheet in wb.sheets():
            ...
            for rownum in xrange(sh.nrows):
                ...
                if header_flag: 
                    ...
                    if header_index == 2 and election_type == u'시도의회의원':
                    ...
                    if column1 in [u"합계", u"관외사전투표", u"거소우편투표", u"잘못투입·구분된투표지", u"소계"]:
