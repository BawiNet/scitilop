...
for file_in_dir in files_in_dir:
    ...
    if m:
        ...
        if len(info_from_filename) == 2 and not (election_type == u'기초의원비례대표선거' or election_type == u'구시군의장선거'):
            # 광역레벨의 경우
            ...
            # 기초의원비례대표 또는 구시군장.
            sgg_name = info_from_filename[1][3:] # 01_천안시 이런 식 
            ...
            sgg_name      = info_from_filename[1][3:]  # 3_시도의회의원선거-01_천안시서북구-천안시제5선거구
            precinct_name = info_from_filename[2]      #
            election_level = 2
            column_offset = -1

            # 그런데 천안시의 경우에는 천안시서북구와 천안시동남구로 분류가 되어 있어 여기 걸린다.
            if sgg_name == u"천안시":
                sgg_name = info_from_filename[2][2:]
                    ...
                    if (values[3+column_offset] == "" ) and values[6+column_offset][:4] != "": 
                        ...
                        for cn in values[5+column_offset:-3+bigo_offset]:
                            ...
                            if len(cn.split("\n")) > 1 and not split_flag:
                                ...
                                if election_type == u"광역의원비례대표선거" or election_type == u'기초의원비례대표선거':
                                    party = cn.replace("\n", "")
                                    name  = party
                                else:
                            ...
                            if election_level == 1 or election_type == u"기초의원비례대표선거": #광역레벨일 경우에 그렇게 써줘야한다.
